...
namespace BlueMystic
{
    ...
    public class DarkModeCS
    {
        ...
        public static void SetRoundBorders(Control _Control, int Radius = 10, Color? borderColor = null,
            int borderSize = 2, bool underlinedStyle = false)
        {
            try
            {
                    ...
                    _Control.Paint += (object sender, PaintEventArgs e) =>
                    {
                        ...
                        if (Radius > 1) //Rounded TextBox
                        {
                            ...
                            using (Pen penBorder = new Pen((Color)borderColor, borderSize))
                            {
                                ...
                                {
                                    using (GraphicsPath pathTxt =
                                           GetFigurePath(_Control.ClientRectangle, borderSize * 2))
                                    {
                                ...
                                if (underlinedStyle) //Line Style
                                {
                                    ...
                                    graph.DrawLine(penBorder, 0, _Control.Height - 1, _Control.Width,
                                        _Control.Height - 1);
                                }
                                else //Normal Style
                                {
        ...
        private static void ApplySystemDarkTheme(Control control = null)
        {
            ...
            if (DwmSetWindowAttribute(control.Handle, (int)DWMWINDOWATTRIBUTE.DWMWA_USE_IMMERSIVE_DARK_MODE_BEFORE_20H1,
                    DarkModeOn, 4) != 0)
                DwmSetWindowAttribute(control.Handle, (int)DWMWINDOWATTRIBUTE.DWMWA_USE_IMMERSIVE_DARK_MODE, DarkModeOn,
                    4);
        ...
        private static Color GetReadableColor(Color backgroundColor)
        {
            ...
            return luminance < 0.5
                ? Color.FromArgb(182, 180, 215)
                : Color.FromArgb(34, 34, 34); // Dark gray for light backgrounds
        }
